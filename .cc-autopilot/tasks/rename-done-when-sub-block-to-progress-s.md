...
### 2026-05-25 — blocked
Retry of prior blocked attempt — no new commit warranted. Code-side rename (scope items 1, 2, 4, 5, 6) is fully landed in ac4f861 and intact: ! grep -rq '_DONE_WHEN' ap2/ → OK, ! grep -rqE 'done_when_bullets|has_done_when' ap2/ → OK, grep -q '_PROGRESS_SIGNALS' ap2/goal.py → OK, grep -qE 'progress_signals_bullets|has_progress_signals' ap2/goal.py → OK, full ap2/tests/ suite 1997 passed (97s). The two goal.md verification bullets (! grep -qE '^Done when:|^### Done when' goal.md and grep -qE '^Progress signals:|^### Progress signals' goal.md) still FAIL — goal.md L213 inside the second '## Current focus:' heading ('operator-legible reporting and monitoring') still reads 'Done when:' verbatim. Scope item (3) requires editing goal.md, which is in ap2/tools.py:TASK_AGENT_FENCED_PATHS (L1244), and the only legitimate write path — the operator-queue 'update_goal' op — is operator-CLI-only by design per ap2/operator_queue.py:389-395 (MM-handler queue-append wrapper refuses it; agents are explicitly directed to surface the recommendation in RESULT and let the operator apply via `ap2 update-goal`). CLAUDE.md prohibits the Bash workaround. This is a structural conflict between briefing scope (3) and the goal.md fence, not an auto-unfixable briefing-shape regression (no AP2_AUTO_UNFREEZE_FIX_SHAPES match). Operator action required — one of: (a) run `ap2 update-goal --file <new-goal.md>` against a goal.md whose L213 has 'Done when:' rewritten to 'Progress signals:' (no bullet content changes); the parser is already wired for the new heading and will pick it up on the next focus-list read — or (b) `ap2 update --briefing-file ...` to drop scope item (3) and the two goal.md verification bullets from TB-285's briefing if the rename is deferred. Note: the pyc-stale-bytecode trap that briefly false-positived the ap2/ greps mid-run (stale .pyc in __pycache__/ contained pre-rename interned strings) is self-clearing — running pytest regenerates clean bytecode under the renamed source, and __pycache__/ is gitignored, so no commit is needed for that.
- **commit:** ac4f861
- **Debug dumps:** `prompt: .cc-autopilot/debug/20260525T064827Z-TB-285.prompt.md`, `stream: .cc-autopilot/debug/20260525T064827Z-TB-285.stream.jsonl`, `messages: .cc-autopilot/debug/20260525T064827Z-TB-285.messages.jsonl`
### 2026-05-25 — blocked
Third attempt — code-side rename (scope items 1, 2, 4, 5, 6) remains fully landed in ac4f861 and intact: `! grep -rq '_DONE_WHEN' ap2/` PASS, `! grep -rqE 'done_when_bullets|has_done_when' ap2/` PASS, `grep -q '_PROGRESS_SIGNALS' ap2/goal.py` PASS, `grep -qE 'progress_signals_bullets|has_progress_signals' ap2/goal.py` PASS, full `uv run pytest -q ap2/tests/` 1997 passed (98s). Scope item (3) BLOCKED on structural fence: goal.md L213 is the sole remaining `^Done when:` per-focus sub-block heading, but goal.md is in `ap2/tools.py:TASK_AGENT_FENCED_PATHS` (L1244) and the SDK rejects Edit/Write; CLAUDE.md prohibits Bash workarounds; the only legitimate write path (operator-queue `update_goal` op) is operator-CLI-only by design (ap2/operator_queue.py L389-395 documents the contract: "handlers that think goal.md needs updating raise the recommendation in their RESULT summary; the operator applies via `ap2 update-goal`"). No `AP2_AUTO_UNFREEZE_FIX_SHAPES` match — the two failing verification bullets (`! grep -qE '^Done when:|^### Done when' goal.md` and `grep -qE '^Progress signals:|^### Progress signals' goal.md`) are valid shell, just structurally unsatisfiable from inside a task agent. **Operator action required** — one of: (a) write a copy of goal.md with L213 rewritten from `Done when:` to `Progress signals:` (no bullet-content changes; the prose mentions of `Done when:` at L120/L122 are descriptive and don't match the `^Done when:` regex, so are not blocking), then `ap2 update-goal --file <new-goal.md>` — parser is already wired for the new heading and picks it up on next focus-list read; OR (b) `ap2 update --briefing-file ...` to drop scope item (3) and the two goal.md verification bullets so the existing ac4f861 commit can close out as complete. Recommendation: (a) — it's a one-line operator edit and closes the briefing cleanly.
- **commit:** ac4f861
- **Debug dumps:** `prompt: .cc-autopilot/debug/20260525T065614Z-TB-285.prompt.md`, `stream: .cc-autopilot/debug/20260525T065614Z-TB-285.stream.jsonl`, `messages: .cc-autopilot/debug/20260525T065614Z-TB-285.messages.jsonl`
