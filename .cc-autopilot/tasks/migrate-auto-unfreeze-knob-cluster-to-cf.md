...
this task applies the same template to `auto_unfreeze`. Per
FLAT_TO_SECTIONED in `ap2/config_compat.py` (TB-323, a50e686), the
auto_unfreeze cluster owns 5 sectioned mappings under
`components.auto_unfreeze`: `disabled`, `fix_shapes`, `dry_run`,
`max_per_task`, `max_per_day`. TB-322 (e38bb38) already declared the
schema on `ap2/components/auto_unfreeze/manifest.py`; this task
...
- Migrate every `os.environ.get("AP2_AUTO_UNFREEZE_...")` call site
  in `ap2/components/auto_unfreeze/__init__.py` and
  `ap2/components/auto_unfreeze/manifest.py` to read from the
  resolved config via `Config.get_component_value("auto_unfreeze",
  <key>)` per the TB-326 pilot pattern. The 5 keys per
  FLAT_TO_SECTIONED: `disabled`, `fix_shapes`, `dry_run`,
  `max_per_task`, `max_per_day`. Cross-check against the manifest's
...
- Preserve existing env-override semantics — TB-323's
  ...
  sectioned-`AP2_COMPONENTS_AUTO_UNFREEZE_FOO` into the resolved
...
- New regression-pin test
  `ap2/tests/test_tb327_auto_unfreeze_cfg_reads.py`: grep-walk
  asserts zero remaining `os.environ.get("AP2_AUTO_UNFREEZE_...")`
  calls in `ap2/components/auto_unfreeze/`; one behavioral test per
  migrated knob asserts the cfg read returns the same value the env
  read would have returned.
- Daemon tick-hook and any external callers stay unchanged
  (`auto_unfreeze.sweep()` signature preserved).

## Design

Adopt the TB-326 option-2 pattern verbatim:
`Config.get_component_value("auto_unfreeze", "<key>")`. The helper
already exists on `Config` (landed in TB-326's commit b3eba54 per
the manifest documentation requirement); this task reuses it
without redesign. Behavior preservation contract: every test that
exercises auto_unfreeze today (covering kill-switch, dry-run,
fix-shapes, per-task / per-day caps) passes without modification.
...
- `uv run pytest -q ap2/tests/test_tb327_auto_unfreeze_cfg_reads.py`
  — new cluster test passes.
- `! grep -rqE "os\.environ\.get\(.AP2_AUTO_UNFREEZE_" ap2/components/auto_unfreeze/`
  — zero remaining direct env reads of AP2_AUTO_UNFREEZE keys in
  the component body (passes iff the grep finds zero matches, per
  TB-270 absence-check convention).
- `grep -rE "get_component_value\(.auto_unfreeze." ap2/components/auto_unfreeze/`
  — the new resolved-config read path is present.
- `uv run python -m ap2 status --project .` exits 0 and the
  `## Components` block still renders `auto_unfreeze` correctly
  (sanity check the cfg read path didn't break the status
  enumeration).

## Out of scope

- The other 4 component clusters (attention, focus_advance,
