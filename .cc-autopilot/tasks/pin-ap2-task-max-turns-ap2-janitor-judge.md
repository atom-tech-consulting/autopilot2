...
- Test-function naming convention: `def test_<knob_lowercase>_<aspect>(...)` — e.g. `test_task_max_turns_default`, `test_janitor_judge_max_turns_invalid`, `test_mm_team_id_unset_raises`. This convention is what the auto-verify in Verification bullet #3 greps for; deviating from it will fail the bullet.

## Verification

- `uv run pytest -q ap2/tests/` — full suite passes after the change (exit 0).
- `uv run pytest -q -k "tb210 or task_max_turns or janitor_judge_effort or janitor_judge_max_turns or mm_team_id"` — the new tests are collected and pass (exit 0); pytest's `-k` expression matches whichever file the implementer chose.
- `[ "$(grep -hE 'def test_(task_max_turns|janitor_judge_effort|janitor_judge_max_turns|mm_team_id)_' ap2/tests/test_env_knobs.py ap2/tests/test_tb210_env_knobs.py 2>/dev/null | wc -l)" -ge 4 ]` — at least 4 test functions exist across the candidate test files (one happy-path per knob minimum), regardless of whether the implementer added a new file or extended the existing one. `2>/dev/null` swallows "no such file" errors so the missing-file branch doesn't fail.
- `[ "$(grep -lE 'AP2_TASK_MAX_TURNS|AP2_JANITOR_JUDGE_EFFORT|AP2_JANITOR_JUDGE_MAX_TURNS|AP2_MM_TEAM_ID' ap2/tests/*.py | wc -l)" -ge 2 ]` — at least 2 test files reference these four env knobs (test_coverage_drift.py + at least one other — the new module or extended existing one). Glob `ap2/tests/*.py` expands shell-side so `grep -l` operates on files, not a directory (no `-r` needed).
...
- `[ "$(grep -cE 'AP2_(TASK_MAX_TURNS|JANITOR_JUDGE_EFFORT|JANITOR_JUDGE_MAX_TURNS|MM_TEAM_ID)' ap2/tests/test_coverage_drift.py)" -le 4 ]` — at most 4 occurrences of these knob names remain in test_coverage_drift.py (zero is target — comment-block enumeration removed/trimmed; up to 4 accommodates a single audit-comment line that lists the knob names without per-knob explanation). The contract: the shim no longer carries these names as the ONLY reference.
- Prose: the new tests assert on actual call-site behavior — not just `os.environ.get` round-trips — for each of the four knobs. Judge confirms by reading the new test bodies and checking that each test references the call-site module symbol (e.g. `daemon._build_task_options`, `janitor._judge`, `sandbox.install_channel` or equivalent) rather than just stub-asserting on env state.
