...
(1) **New CLI subcommand `ap2 audit`** in `ap2/cli.py`. Default mode lists unreviewed shipped tasks; flags add interactive walkthrough, JSON output, and cursor reset.
  ...
  - `--interactive`: walks through each unreviewed task one at a time. For each, displays full task summary + the task_complete event's `summary` field + auto_approved status + briefing path. Prompt: `[c]lassify | [s]kip | [n]ext | [q]uit`. `c` triggers a sub-prompt for `--impact <verdict>` (must be one of `IMPACT_VERDICTS = ("advanced-goal", "pro-forma", "unclear")` per `tools.py:586`) + optional `--reason TEXT`, then queues `ap2 classify TB-N --impact <verdict> --reason "..."` through the operator queue (re-uses existing CLI helper, doesn't duplicate the queue-append code). `s` queues an `audit_skip` op (see Scope §3) → daemon drain appends `<ts> — audit-skipped TB-N: <reason>` to `operator_log.md`. `n` advances to next task without recording anything (operator wants to think later). `q` exits, recording a `ran audit (reviewed M, skipped K, deferred L)` line to operator_log.md. **Rollback intentionally out of scope** — see Out-of-scope below.
...
(2) **State source: `operator_log.md` only** (no new state file). The audit cursor + reviewed-set are both derived from grep:
  ...
  - **Reviewed set**: union of (a) tasks with a `<ts> — classified TB-N impact=<verdict>` line (existing TB-189 writer; verdict is one of `advanced-goal` / `pro-forma` / `unclear`), (b) tasks with a `<ts> — audit-skipped TB-N` line (new writer per §3), (c) tasks with a `<ts> — rejected TB-N` line (existing reject writer — explicit operator decision).
...
(4) **Tests** (`ap2/tests/test_audit_cmd.py`):
  ...
  - `test_audit_classified_task_excluded`: task with `<ts> — classified TB-N impact=advanced-goal: ...` in operator_log → excluded from unreviewed set.
...
- **Rollback as an interactive-prompt action** (`[r]ollback`). Deferred to a follow-up TB after `ap2 audit` lands and operator-engagement reveals which rollback shape is actually wanted: walk-back-N (existing `ap2 rollback` semantics), rollback-this-specific-TB (requires computing commit-before-TB-N), or revert-and-classify-as-pro-forma (combines two operator decisions into one). Each has different rollback-semantics risk; bundling into the audit's first iteration would force a premature choice. The operator can still rollback outside the audit walk via the existing `ap2 rollback` CLI; the audit just won't have a one-keystroke shortcut.
