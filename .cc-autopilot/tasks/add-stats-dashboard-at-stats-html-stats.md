...
- `[ "$(grep -hE 'window' ap2/automation_status.py ap2/automation_stats.py 2>/dev/null | wc -l)" -ge 3 ]` — at least 3 mentions of "window" across the collector code (window-param parsing, window-bounded filter, window default). Sanity check that windowing is implemented, not stubbed. Note: uses `grep -hE | wc -l` (output matching lines, count them) rather than `grep -cE` (which with multiple files emits per-file `file:N` lines that fail integer comparison).
