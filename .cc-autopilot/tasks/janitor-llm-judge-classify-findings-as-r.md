...
- `! grep -nE "operator_log_append|operator_log.md" ap2/janitor.py` — bash-negated grep; exits 0 only if there are ZERO matches (regression check: TB-177's operator_log.md write is gone). Inverted so the verifier — which only treats exit 0 as pass (`ap2/verify.py:266`) — interprets "no matches" correctly; a bare `grep` with no matches exits 1 and would trip the verifier even though the regression intent is satisfied.
