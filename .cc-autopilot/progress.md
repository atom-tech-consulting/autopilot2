...
## [2026-05-30] TB-346: Config-correctness cleanup: component-value resolution + inline-default consistency + auto_diagnose schema coverage
- **Commit:** `1fae83d`
- **Summary:** Previously committed in 1fae83d (this is the retry); verified it fully covers all 3 scope items — (1) config_introspect._resolve_component_value now routes through cfg.get_component_value(comp,key,default=schema.default), the TB-344 component-branch twin; (2) ideation.py + ideation_scrub.py dropped the redundant inline default="" so both rely on CORE_CONFIG_SCHEMA defaults (False / DEFAULT_IDEATION_SCRUB_MODEL), behavior-preserving; (3) auto_diagnose_cooldown_s + auto_diagnose_idle_threshold_s added to CORE_CONFIG_SCHEMA (int, defaults from the DEFAULT_AUTO_DIAGNOSE_* constants) + howto.md entries. Audit: all 5 briefing shape-greps pass, both new component regression tests + 61 ideation/config tests pass, deterministic suite 2793/2794. ROOT CAUSE of the prior per_task verification_failed (NOT a code defect): the briefing's `uv run --extra dev pytest -q ap2/tests/` bullet runs the live-SDK smoke dir, and ap2/tests/smoke/test_prose_judge_real_sdk.py is currently failing with a transient SDK transport error ('Claude Code returned an error result: success') in verify._judge_prose_bullet — a path TB-346 never touches. The operator's AP2_VERIFY_CMD `--ignore=ap2/tests/smoke` fix covers ONLY the project-wide gate (_run_verify); the per-task verifier (_maybe_per_task_verify→verify_task) runs briefing shell bullets verbatim, so the smoke-inclusive pytest bullet still gates this task. RECOMMEND operator align the briefing's pytest bullet with --ignore=ap2/tests/smoke (or tag TB-346 #no-verify) so per-task verification is deterministic — briefing/env/verify.py are all out of my scope/fenced.
- **Files:** ap2/config_introspect.py, ap2/core_config_schema.py, ap2/ideation.py, ap2/ideation_scrub.py, ap2/howto.md, ap2/tests/test_tb324_cli_config.py
- **Tests:** pass

## [2026-05-30] TB-350: Add a 6-hourly real-SDK smoke cron job dispatched through a dedicated routine
- **Commit:** `ff1612c`
...
## [2026-05-31] TB-361: Fix the TB-356 thinking-block classifier (never matches the real failure) and exempt that class from the auto-approve breaker
- **Commit:** `9baa2f5`
- **Summary:** Fixed all three thinking-block recovery defects — classifier now walks message text (`text_preview`/`result`/content-block `text`) so it matches both the recorded dict `last_messages` shape AND raw SDK message objects (the latter returned False before, the real gap); the `task_error` event carries a `thinking_block_corruption` flag that exempts the class from the TB-224 breaker + TB-223 freeze scan; and the auto-promote step now iterates dispatchable Backlog candidates (Board.iter_dispatchable + _auto_promote_gate_halts) to skip past a gated head and promote non-gated work behind it — full suite 2926 passed. Note: root-cause refinement — stream_log has been summary-dicts since TB-85 (not raw objects), so the dict-shape classifier already matched; production effort_downshift=0 was because TB-354 predates the TB-356 commit and the live daemon hadn't restarted onto TB-356 code, but the raw-object fragility the briefing feared was real and is now closed.
- **Files:** ap2/board.py, ap2/components/auto_approve/impl.py, ap2/daemon.py, ap2/tests/test_tb361_thinking_block_recovery.py
- **Tests:** pass

## [2026-05-31] TB-360: Axis 6 canary — migrate ideation-scrub (`_run_scrub`) to adapter-routing + per-kind backend selection
- **Commit:** `fc5db75`
...
## [2026-06-08] TB-381: Pipeline component + extended tick-phase vocabulary (axis 1 canary)
- **Commit:** `29c3fe6`
- **Summary:** Extracted the cron scheduler into a registry tick-hook component (ap2/components/cron/, Phase.CRON_DISPATCH) with a job-handler registry replacing run_cron's job.name switch, added the reserved Phase.IDEATION, kept the interval engine + _run_control_agent + status-report handler in core, and the import-direction gate stays green; full suite passes (3010 tests).
- **Files:** ap2/registry.py, ap2/daemon.py, ap2/cron_handlers.py, ap2/components/cron/__init__.py, ap2/components/cron/impl.py, ap2/components/cron/manifest.py, ap2/components/janitor/impl.py, ap2/components/janitor/manifest.py, ap2/components/janitor/__init__.py, ap2/config_compat.py, ap2/howto.md, ap2/init.py, ap2/tests/test_tb381_cron_component.py, ap2/tests/test_tb310_tick_hook_protocol.py, ap2/tests/test_tb211_event_types.py, ap2/tests/test_janitor.py, ap2/tests/test_smoke_runner.py, ap2/tests/test_status_report_skip.py, ap2/tests/test_ideation_halt.py, ap2/tests/test_tb223_auto_approve.py, ap2/tests/test_tb224_token_caps.py, ap2/tests/test_tb225_auto_unfreeze.py, ap2/tests/test_tb272_validator_judge_noisy_pause.py, ap2/tests/test_tb361_thinking_block_recovery.py
- **Tests:** pass

## [2026-06-08] TB-382: Extract the prose-judge into a verifier_judge component (axis 5)
- **Commit:** `e98d4d1`
...
## [2026-06-09] TB-385: Slim the per-task lifecycle events: rename `task_start`→`task_solve`, fold `verify_passed`+`judge_call` into one terminal `task_verify`
- **Commit:** `d8c8865`
- **Summary:** Renamed task_start→task_solve and folded verify_passed + per-bullet judge_call into one terminal task_verify event (verdict/shell/prose/verify_cmd/bullets), emitted on all outcomes from run_task and the pipeline sweep; migrated all consumers (attention detector, automation_stats, doctor verify-timeout audit, event_monitor/diagnose, web, status-report, howto event schema) to derive from task_verify while tolerating legacy events; full suite green (3027 passed).
- **Files:** ap2/daemon.py, ap2/verify.py, ap2/pipeline_sweep.py, ap2/components/verifier_judge/impl.py, ap2/components/attention/impl.py, ap2/components/janitor/impl.py, ap2/automation_stats.py, ap2/doctor.py, ap2/event_monitor.py, ap2/diagnose.py, ap2/web_chrome.py, ap2/web_events.py, ap2/status_report.py, ap2/howto.md, ap2/tests/e2e/test_verify_per_task.py, ap2/tests/test_judge_parse_observability.py, ap2/tests/e2e/test_single_tick.py, ap2/tests/e2e/test_review_gate.py, ap2/tests/e2e/test_multi_tick_cron.py, ap2/tests/e2e/test_mattermost_cron.py, ap2/tests/e2e/test_walk_away_loop.py, ap2/tests/test_daemon_recovery.py
- **Tests:** pass

## [2026-06-09] TB-386: Demote both LLM judges from components to select_adapter layers
- **Commit:** `6cab646`
- **Summary:** Demoted both LLM judges from components back to core select_adapter layers: moved _judge_prose_bullet into verify.py (gated by plain AP2_VERIFY_JUDGE_DISABLED) and the dep-coherence judge into briefing_validators.py (called directly via select_adapter), deleted ap2/components/{verifier,validator}_judge/ plus registry.verifier_judge()/briefing_validators(), rewired tools.py+doctor.py to plain imports, preserved both off-switches, and updated the CI-gate/disabled-config/schema/cfg-read tests + howto/architecture docs; full suite 3015 passed.
- **Files:** ap2/verify.py, ap2/briefing_validators.py, ap2/registry.py, ap2/tools.py, ap2/doctor.py, ap2/config_compat.py, ap2/env_reload.py, ap2/init.py, ap2/howto.md, ap2/architecture.md, ap2/components/verifier_judge/ (deleted), ap2/components/validator_judge/ (deleted), ap2/tests/test_tb382_verifier_judge_component.py, ap2/tests/test_tb316_validator_pipeline.py, ap2/tests/test_tb331_validator_judge_cfg_reads.py, ap2/tests/test_tb322_component_schemas.py, ap2/tests/test_tb334_core_cfg_reads.py, ap2/tests/test_components_disabled.py, ap2/tests/test_tb269_validator_judge_timeout_calibration.py, ap2/tests/test_tb270_validator_judge_payload_slice.py, ap2/tests/smoke/test_validator_judge_real_sdk.py
- **Tests:** pass

## [2026-06-09] TB-387: One generic contributions(point) registry accessor; delete bespoke per-kind methods
- **Commit:** `085f83e`
...
## [2026-06-09] TB-393: Docs: document the agent-backend layer (Claude Code default + Codex) across architecture.md, howto.md, README
- **Commit:** `25703f7`
...
## [2026-06-12] TB-407: Cleanup: repoint/remove stale `ap2/howto.md` references in code + test comments (howto.md was retired by TB-406)
- **Commit:** `8186bda`
- **Summary:** Swept ap2/ (code + tests) so no `howto.md` literal remains: repointed navigational comments/docstrings to the owning carve skills (ap2-observability/-config/-board-ops/-task/-ideation-goals) or architecture.md, removed bare provenance citations to the deleted file, and preserved test_sync_assets.py's three deploy-absence guards via a parts-assembled constant (assertion behavior unchanged); HOWTO_PATH constant already absent.
- **Files:** ap2/_shared.py, ap2/cli_config.py, ap2/components/attention/impl.py, ap2/components/auto_approve/manifest.py, ap2/config_loader.py, ap2/core_config_schema.py, ap2/events.py, ap2/init.py, ap2/prompts.py, ap2/verify.py, ap2/tests/_source_registry.py, ap2/tests/test_docs_drift.py, ap2/tests/test_coverage_drift.py, ap2/tests/test_sync_assets.py, ap2/tests/test_docs.py, ap2/tests/test_verify_classifier.py, ap2/tests/test_ideation_defaults.py, ap2/tests/test_tb229_briefing_fix_teaching.py, ap2/tests/test_tb244_status_report_focus_rotation_digest.py, ap2/tests/test_tb245_status_report_validator_judge_digest.py, ap2/tests/test_tb259_status_report_stats_window.py, ap2/tests/test_tb273_ideation_pitfalls_sync.py, ap2/tests/test_tb287_attention_task_frozen.py, ap2/tests/test_tb288_attention_validator_judge_noisy.py, ap2/tests/test_tb289_attention_auto_approve_paused.py, ap2/tests/test_tb290_attention_cost_cap_approach.py, ap2/tests/test_tb297_attention_immediate_push.py, ap2/tests/test_tb322_component_schemas.py, ap2/tests/test_tb324_cli_config.py, ap2/tests/test_tb336_axis5_tail_cfg_reads.py, ap2/tests/test_tb337_core_schema.py, ap2/tests/test_tb396_provider_neutral_model.py
- **Tests:** pass

## [2026-06-16] TB-408: License wiring: PolyForm Noncommercial 1.0.0 LICENSE + pyproject license field & classifiers
- **Commit:** `16faf33`
...
## [2026-06-22] TB-422: Ship operator skills with the installed package so sync-assets works after `uv tool install` (not only from a repo clone)
- **Commit:** `8e58554`
- **Summary:** Relocated operator skills to ap2/skills/ shipped as package data and switched _skills_source() to importlib.resources resolution with a repo-relative fallback, so sync-assets deploys the skills after a non-editable uv tool install (verified the built wheel ships all 8 SKILL.md bundles); full non-smoke suite green (3108 passed, 1 skipped).
- **Files:** ap2/sandbox.py, pyproject.toml, MANIFEST.in, README.md, ap2/skills/** (moved from skills/**), ap2/tests/test_skills_packaging.py, ap2/tests/test_packaging.py, ap2/tests/test_sync_assets.py, ap2/tests/test_docs_drift.py, ap2/tests/test_docs.py, ap2/tests/test_no_sandbox_path_leak.py, ap2/tests/test_tb135_verification.py, ap2/tests/test_tb229_briefing_fix_teaching.py, ap2/tests/test_tb244_status_report_focus_rotation_digest.py, ap2/tests/test_tb245_status_report_validator_judge_digest.py, ap2/tests/test_tb259_status_report_stats_window.py, ap2/tests/test_tb273_ideation_pitfalls_sync.py, ap2/tests/test_tb287_attention_task_frozen.py, ap2/tests/test_tb288_attention_validator_judge_noisy.py, ap2/tests/test_tb289_attention_auto_approve_paused.py, ap2/tests/test_tb290_attention_cost_cap_approach.py, ap2/tests/test_tb324_cli_config.py, ap2/tests/test_tb336_axis5_tail_cfg_reads.py
- **Tests:** pass

## [2026-06-23] TB-423: Remove the statusline feature from repo and code (it references a deleted hooks/ script and is not required by ap2)
- **Commit:** `2393375`
