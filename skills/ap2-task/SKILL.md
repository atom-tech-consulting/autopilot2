...
- **Canonical structure (TB-154).** The briefing must contain ALL FIVE `##`-level sections — `## Goal`, `## Scope`, `## Design`, `## Verification`, `## Out of scope` — by exact name. The queue-append-time validator (`_validate_briefing_structure` in `ap2/tools.py`) rejects briefings missing any of these BEFORE TB-N is allocated. Section order is free; extension (extra `##` sections like `## Decision log`) is allowed; renaming (`## Acceptance` for `## Verification`) is not.
