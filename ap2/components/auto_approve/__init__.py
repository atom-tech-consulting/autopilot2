...
TB-318 (axis 5): relocated from the flat module path
`ap2/auto_approve.py` to this subpackage so core no longer statically
imports the gate-evaluation surface. The daemon resolves the
module-level aliases above through
`default_registry().get("auto_approve").hook_points[…]` (see
`ap2/components/auto_approve/manifest.py`). The inline per-task gate
block in `daemon._tick` stays inline this cycle — extracting it to a
tick-callable carries observable-payload risk and is a separate
follow-up.
"""
