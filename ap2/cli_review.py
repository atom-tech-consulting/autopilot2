...
def cmd_rewind_focus(cfg: Config, args: argparse.Namespace) -> int:
    """Re-engage an exhausted `## Current focus:` heading (TB-295).

    Operator surface for recovering from a falsely-advanced focus —
    today's recovery shape (direct `focus_pointer.json` edit) produces
    no event and leaves pre-rewind empty cycles counting against the
    rewound focus's counter (the empty-cycles counter cuts its window
    at the most recent `focus_advanced to=<focus_title>` event, so no
    event = no cutoff = walk the whole tail). This verb queues a
    `rewind_focus` op that the drain-side handler applies at the next
    tick boundary: pointer mutation + synthetic `focus_advanced
    trigger=operator_rewind` event (the cutoff anchor) + an
    `operator_log.md` audit line, all under `board_file_lock`.

    Routed through the operator queue (rather than mutating the pointer
    directly here) so the write lands at a tick boundary and never
    races an in-flight ideation cycle's pointer read. Same rationale
    as `ap2 update-goal` / `ap2 approve`.

    Title-as-key (not index-as-key) by design: operators don't
    reliably remember foci by numeric index; titles are stable text in
    goal.md. The handler resolves title → index at DRAIN time (not CLI
    time), so an operator-edited goal.md between this invocation and
    the next tick produces a clean rejection rather than a silent
    rewind to the wrong index. Snapshot-validate here too so an
    obviously-wrong title surfaces immediately at CLI exit.
    """
    ...
    if not title:
        print(
            "ap2 rewind-focus: <title> is required",
            file=sys.stderr,
        )
        return 1

    # Snapshot-validate: the title must match one of the current
    # `## Current focus:` headings. Re-resolved at drain time too;
    # this is the early-rejection surface so operator typos don't
    # silently queue a doomed op.
    ...
    if title not in titles:
        if titles:
            ...
            available = "(none — goal.md has no `## Current focus:` headings)"
        print(
            f"ap2 rewind-focus: title {title!r} does not match any "
            f"`## Current focus:` heading in goal.md.\n"
            f"  Available titles: {available}",
            file=sys.stderr,
        )
        return 1

    payload: dict = {"op": "rewind_focus", "title": title}
    ...
    if res.get("isError"):
        ...
        return 1
    print(
        f"queued rewind_focus → {title!r} "
        f"(pointer update + synthetic focus_advanced will land at "
        f"next tick)"
    )
    return 0
