...
# TB-386 (axis 5a): the LLM dep-coherence judge surface was demoted out of
# the `ap2/components/validator_judge/` component back into the core
# briefing-validation runner (`ap2/briefing_validators.py`). `tools.py`
# re-exports the same private attribute names (`_DEP_JUDGE_PARSE_ERRORS`,
# `_DepJudgeOutcome`, …) verbatim via a plain import so
# `from ap2.tools import _DepJudgeTimeout` etc. keeps working in the >50
# test modules that touch this surface. The pre-TB-386 lazy `__getattr__`
# registry-resolution shim is gone — `briefing_validators` is already
# imported at module load (above), so a direct import carries no extra
# circular-import risk.
from .briefing_validators import (  # noqa: E402
