...
| `ap2 sandbox user-setup [USER] [-y] [--skip-token] [--mm-url/--mm-token]` | Create the sandbox user (prompts before running sudo). | One-time per machine; pairs with `install-token` / `sync-assets` / `install-mm` to fill in creds + per-user config (TB-276 folded the prior `install-howto` step into `sync-assets`). Skip flags exist for partial setups. |
