...
# ---------------------------------------------------------------------------
# install_statusline (TB-78)


class _FakeRun:
    """subprocess.run stub for the install_statusline command set.

    Recognises:
      - sudo -u <user> mkdir -p <path>          → rc 0
      - sudo -u <user> tee <path> (input kw)    → captures (path, content)
      - sudo -u <user> chmod <mode> <path>      → captures (mode, path)
      - sudo -u <user> sh -c "test -f X && cat X || echo {}"
                                                → returns settings_text
    """

    def __init__(self, settings_text: str = "") -> None:
    ...
    def __call__(self, argv, *a, **kw):
        if "tee" in argv:
            tee_idx = argv.index("tee")
            path = argv[tee_idx + 1]
            self.captures.append((path, kw.get("input", "")))
            return subprocess.CompletedProcess(argv, 0, stdout="", stderr="")
        if "chmod" in argv:
            ci = argv.index("chmod")
            self.chmods.append((argv[ci + 1], argv[ci + 2]))
            return subprocess.CompletedProcess(argv, 0, stdout="", stderr="")
        if "mkdir" in argv:
            return subprocess.CompletedProcess(argv, 0, stdout="", stderr="")
        if "sh" in argv and any("settings.json" in str(t) for t in argv):
            return subprocess.CompletedProcess(
                argv, 0, stdout=self.settings_text or "{}", stderr="",
            )
        return subprocess.CompletedProcess(argv, 0, stdout="", stderr="")


def _wire_statusline(monkeypatch, tmp_path: Path, fake_run: _FakeRun) -> Path:
    home = tmp_path / "agent-home"
    home.mkdir()
...
def test_install_statusline_writes_script_and_settings(monkeypatch, tmp_path):
    fake = _FakeRun(settings_text="")  # no existing settings.json
    home = _wire_statusline(monkeypatch, tmp_path, fake)

    rc = sandbox.install_statusline("claude-agent")
    assert rc == 0

    paths = [p for p, _ in fake.captures]
    assert str(home / ".claude" / "statusline-command.sh") in paths
    assert str(home / ".claude" / "settings.json") in paths

    settings_capture = next(c for p, c in fake.captures if p.endswith("settings.json"))
    import json as _json
    parsed = _json.loads(settings_capture)
    assert parsed["statusLine"]["type"] == "command"
    assert parsed["statusLine"]["command"].endswith("statusline-command.sh")
    assert "bash " in parsed["statusLine"]["command"]
    # Script was chmod 0755.
    assert any(m == "0755" for m, _ in fake.chmods)


def test_install_statusline_preserves_existing_settings_keys(monkeypatch, tmp_path):
    """Existing keys (permissions, model, etc.) must survive the merge."""
    existing = '{"permissions": {"defaultMode": "auto"}, "model": "opus[1m]"}'
    fake = _FakeRun(settings_text=existing)
    _wire_statusline(monkeypatch, tmp_path, fake)

    sandbox.install_statusline("claude-agent")

    settings_capture = next(c for p, c in fake.captures if p.endswith("settings.json"))
    import json as _json
    parsed = _json.loads(settings_capture)
    assert parsed["model"] == "opus[1m]"
    assert parsed["permissions"]["defaultMode"] == "auto"
    assert "statusLine" in parsed


def test_install_statusline_idempotent_when_already_current(monkeypatch, tmp_path, capsys):
    """If statusLine is already correct, no settings.json rewrite happens.
    The script is still copied (cheap, ensures we re-own / re-chmod)."""
    home = tmp_path / "agent-home"
    home.mkdir()
    src = tmp_path / "statusline-command.sh"
    ...
    existing = _json.dumps({
        "statusLine": {"type": "command", "command": f"bash {target_script}"},
        "model": "opus[1m]",
    })
    fake = _FakeRun(settings_text=existing)
    monkeypatch.setattr(sandbox, "_user_exists", lambda u: True)
    ...
    rc = sandbox.install_statusline("claude-agent")
    assert rc == 0
    # Only the script copy was tee'd; settings.json was NOT written.
    settings_writes = [p for p, _ in fake.captures if p.endswith("settings.json")]
    assert settings_writes == []
    out = capsys.readouterr().out
    assert "no-op" in out


def test_install_statusline_refuses_when_user_missing(monkeypatch, capsys):
    monkeypatch.setattr(sandbox, "_user_exists", lambda u: False)
    rc = sandbox.install_statusline("nope")
    assert rc == 1
    assert "does not exist" in capsys.readouterr().err
