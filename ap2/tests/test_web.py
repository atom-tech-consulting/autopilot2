...
#
# A read-path/rendering layer over the three usage-bearing event types
# (`task_run_usage`, `control_run_usage`, `judge_call`). Tests pin:
#   - synthetic 7-day fixture renders the full dashboard end-to-end.
#   - empty-data case renders cleanly with placeholder text.
#   - URL query handling — `?window=` changes the cost-chart range,
#     `?stack=` changes which series populate the chart.
#   - `/` index page links to `/usage`.
#   - per-task aggregation correctly sums task_run_usage + judge_call.
#   - SVG helpers + dispatch + route handler are wired into web.py.
